...
string Purchase::newOrder() {
	...
	stmt.str("");
	stmt << "Select ord_id from purchase where ord_id = " << ordId << ";";
	query = stmt.str();
	q = query.c_str();
	mysql_query(conn, q);
...
string Purchase::markCancel() {
	...
	stmt.str("");
	stmt << "Select received from purchase where ord_id = " << ordId << ";";
	query = stmt.str();
	q = query.c_str();
	mysql_query(conn, q);
	res_set = mysql_store_result(conn);
